# Riverbend Community Garden — website (riverbend-garden)

This repo IS the website for Riverbend Community Garden. There is no dashboard and no CMS: you
change the site by editing these files and pushing. Live at https://riverbend-garden.pages.dev/.
...
- Two pages: `index.html` (hero, mission, events, the volunteer sign-up,
  donate, details) and `donate.html` (how giving works). `404.html` is what
  the host shows for a missing page. This is the **nonprofit / church /
  event** template: the sign-up keeps its rows in the site's own database
  (below); the donate button is only a link to their own giving page.
...
"Done" means live at https://riverbend-garden.pages.dev/, not edited on disk. Relay sessions run from the
client workspace one level up, so git takes the form
`git -C repos/riverbend-garden …`; never `cd` into the repo first (that is always
blocked). After any requested change, without waiting to be asked:

1. Look at your work: `shot repos/riverbend-garden/<page>.html` (add `--mobile` for
   ...
   run `shot check repos/riverbend-garden/<page>.html`. Fix what's off before you
...
2. Commit on `main` with a short plain-English message
   (`git -C repos/riverbend-garden add -A && git -C repos/riverbend-garden commit -m "…"`).
3. Push: `git -C repos/riverbend-garden push`. The repo is the record.
4. Publish: `site publish riverbend-garden`. It sends exactly what is committed to
...
5. Confirm the live site serves the change, with exactly this shape (no
   pipe, no redirect):
       ...
       curl -s https://riverbend-garden.pages.dev/PAGE.html

   (for the home page, `curl -s https://riverbend-garden.pages.dev/`). Read the output and look for the
...
Undo the last change with `git -C repos/riverbend-garden revert HEAD`, push, and
...
A site gets a database with `site data riverbend-garden` (PLAYBOOK § Data). Then
...
|---|---|---|
| Volunteer sign-ups | `submissions` (form `volunteer`) | the sign-up form, `#volunteer` |

The events are plain HTML in `index.html` (`#events`, one `<li>` each):
"add a work day on the 12th" is an edit, a commit and a publish. The donate
button is a link to their own giving page; the site never takes money.
...
`SITE_ADMIN=1 site domain riverbend-garden example.com` from the workspace (adds
...
client at the end is `site transfer riverbend-garden <their-github-user>`.
